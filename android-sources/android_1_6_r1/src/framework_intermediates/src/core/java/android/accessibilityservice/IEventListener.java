...
/**
 * Cast an IBinder object into an IEventListener interface,
...
mRemote.transact(Stub.TRANSACTION_setConnection, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
mRemote.transact(Stub.TRANSACTION_onAccessibilityEvent, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
try {
_data.writeInterfaceToken(DESCRIPTOR);
mRemote.transact(Stub.TRANSACTION_onInterrupt, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
static final int TRANSACTION_setConnection = (IBinder.FIRST_CALL_TRANSACTION + 0);
static final int TRANSACTION_onAccessibilityEvent = (IBinder.FIRST_CALL_TRANSACTION + 1);
static final int TRANSACTION_onInterrupt = (IBinder.FIRST_CALL_TRANSACTION + 2);
}
public void setConnection(android.accessibilityservice.IAccessibilityServiceConnection connection) throws android.os.RemoteException;
public void onAccessibilityEvent(android.view.accessibility.AccessibilityEvent event) throws android.os.RemoteException;
public void onInterrupt() throws android.os.RemoteException;
